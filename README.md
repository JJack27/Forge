...
A ZCode skill that forges a personalized, multi-file HTML learning book (with i18n) from a repo, a topic/domain, or external materials — calibrated to the learner's actual level.
...
- A **multi-file static web project** (one folder per book): `index.html` + `css/` + `js/` + `locales/` + `content/`
...
│   ├── project-structure.md       # multi-file project layout + content schema
...
└── assets/
    └── book-template/             # the template project (copy & fill per book)
        ...
        ├── start.sh
        ├── css/book.css
        ├── js/{i18n,scoring,dashboard,main}.js
        ├── locales/en.json
        └── content/en/{meta.json, ch-01.json}
```
